...
CORES_PRAZO = {
    'Finalizado': '#95A5A6', # Cinza
    'Super Alerta': '#E74C3C', # Vermelho
    'Alerta': '#F4D03F', # Amarelo
    'No Prazo': '#82C45A' # Verde
}

VERSAO_DADOS = 11

def limpar_valor_br(x):
    """Converte valor BR ou numérico do Excel para float correto"""
    if pd.isna(x):
        return 0.0
...
col4.metric("Valor Total Estimado", f"R$ {valor_total:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
...
with col_g1:
    if 'STATUS ENTREGA' in df.columns:
        ...
        fig = px.pie(
            ...
            title=f"Distribuição por Status - {PARAMETRO_CER if PARAMETRO_CER in df['UBS'].unique() else 'Geral'}",
            hole=0.4,
            color='Status',
...
with col_ind2:
    ...
    if 'STATUS ENTREGA' in df.columns and COLUNA_VALOR_TOTAL in df.columns:
        ...
        if total > 0:
            st.metric("Entregue", f"R$ {valor_entregue:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."), f"{valor_entregue/total*100:.1f}%")
            st.metric("Em Aberto", f"R$ {valor_aberto:,.2f}".replace(",", "X").replace(".", ",").replace("X", "."), f"{valor_aberto/total*100:.1f}%")
...
# >>> CONTROLE DE PRAZOS - LENDO CÉLULA O3 <<<
...
st.subheader(
    ...
    f"Valor: R$ {valor_total:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
)
...
st.divider()
st.caption(f"🔄 Dashboard atualiza automaticamente a cada 3 minutos | v2.8 - Botão Limpar Filtros")
