...
col4.metric("Valor Total Estimado", formatar_real(valor_total))
...
with col_ind2:
    ...
    if 'STATUS ENTREGA' in df.columns and COLUNA_VALOR_TOTAL in df.columns:
        ...
        if total > 0:
            st.metric("Entregue", formatar_real(valor_entregue), f"{valor_entregue/total*100:.1f}%")
            st.metric("Em Aberto", formatar_real(valor_aberto), f"{valor_aberto/total*100:.1f}%")
...
st.subheader(
    ...
    f"Valor: {formatar_real(valor_total)}"
)
...
st.divider()
st.caption(f"🔄 Dashboard atualiza automaticamente a cada 3 minutos | v3.2 - Compatível Streamlit Cloud")
